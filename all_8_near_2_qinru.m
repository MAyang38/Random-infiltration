...
lnK = normrnd(2.5, sqrt(0.2), [cell_y cell_x]);
...
%         end
    ...
    if(index >= 2)
       if(S(IP_temp(index - 1,1), IP_temp(index - 1, 2)) == 0)
         Ips = [Ips;IP_temp(index - 1:index - 1,:)];
       end
       S(IP_temp(index - 1,1), IP_temp(index - 1, 2)) =  S(IP_temp(index - 1,1), IP_temp(index - 1, 2)) + 0.1;
...
%     fprintf("%d", Dp);
    ...
    if(index >= 2)
       entry_point = IP_temp(index - 1:index - 1,1:2);
       index_x = entry_point(1);
       index_y = entry_point(2);
       Pd(index_x, index_y) = Pc(index_x, index_y)/(S(index_x, index_y)^(-1/lamdab));
